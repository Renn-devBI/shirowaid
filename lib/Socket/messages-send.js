...
const makeMessagesSocket = (config) => {
    ...
    const shirowaid = new Z(Utils_1, waUploadToServer, relayMessage, config, sock);
    ...
    return {
        ...
        sendStatusMention: async (content, jids = []) => {
            return await shirowaid.sendStatusWhatsApp(content, jids);
        },
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = shirowaid.detectType(content);
            if (typeof content === 'object' && 'disappearingMessagesInChat' in content &&
                ...
                if (messageType) {
                    switch (messageType) {
                        case 'PAYMENT':
                            const paymentContent = await shirowaid.handlePayment(content, quoted);
                            return await relayMessage(jid, paymentContent, {
                                messageId: Utils_1.generateMessageID(),
                                ...getParticipantAttr()
                            });
                        case 'PRODUCT':
                            const productContent = await shirowaid.handleProduct(content, jid, quoted);
                        ...
                        case 'INTERACTIVE':
                            const interactiveContent = await shirowaid.handleInteractive(content, jid, quoted);
                        ...
                        case 'INTERACTIVE_BUTTONS':
                            const ibContent = await shirowaid.handleInteractiveButtons(content, jid, quoted);
                        ...
                        case 'ALBUM':
                            return await shirowaid.handleAlbum(content, jid, quoted)
                        case 'EVENT':
                            return await shirowaid.handleEvent(content, jid, quoted)
                        case 'POLL_RESULT':
                            return await shirowaid.handlePollResult(content, jid, quoted)
                        case 'GROUP_STORY':
                            return await shirowaid.handleGroupStory(content, jid, quoted)
                    }
                }
