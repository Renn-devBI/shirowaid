...
```javascript
await shirowaid.setLabelGroup(jid, string)
```
---
### Delay
Sleep code ( hehe )
...
```javascript
await shirowaid.delay(seconds) // ex. 3 ( 3 seconds )
// example
await shirowaid.delay(3)
```
---
### React Message
...
```javascript
await shirowaid.react(m, emoji) 
// Example 
await shirowaid.react(m, "😚") 
```
---
...
```javascript
await shirowaid.cekIDSaluran(url)
```
Result JSON
```json
{
...
```javascript
await shirowaid.newsletterMultipleFollow(jids)
// Example
await shirowaid.newsletterMultipleFollow("120xxxxxxx@newsletter 120xxxxxxxxx@newsletter 120xxxxxxx@newsletter")
```
---
### Check banned number
...
```javascript
shirowaid.checkBanned(jid)
```
---
### Edit Message
Edit your previously sent message
```js
await shirowaid.edit(m, newText)
// Example
await shirowaid.edit(m, "this is edited message")
```
Notes
...
```javascript
await shirowaid.sendStatusMention(content, jid);
```
        ...
        nativeFlowMessage: {        
            messageParamsJson: JSON.stringify({          
                ...
                tap_target_configuration: {            
                    ...
                    canonical_url: "https://shirowaid.site",            
...
await shirowaid.sendMessage(jid, {
    interactiveMessage: {
        ...
        externalAdReply: {
            ...
            mediaUrl: " X ",
            sourceUrl: "https://shirowaid.site",
        ...
        buttons: [
            {
                ...
                buttonParamsJson: JSON.stringify({
                    ...
                    url: "https://shirowaid.site",
                    merchant_url: "https://shirowaid.site"
                })
            }
        ]
    }
...
```javascript
const resized = await shirowaid.resize(buffer, 200, 200)
```
---
### Convert
...
```javascript
const mp4 = await shirowaid.convert(buffer, { to: "mp4" })
const webp = await shirowaid.convert(buffer, { to: "webp" })
const mp3 = await shirowaid.convert(buffer, { to: "mp3" })
```
---
### To Sticker
...
```javascript
const sticker = await shirowaid.toSticker(buffer)
const sticker = await shirowaid.toSticker(buffer, { quality: 90 })
```
---
### Compress
...
```javascript
const compressed = await shirowaid.compress(buffer, { quality: 50 })
```
---
### Metadata
...
```javascript
const info = await shirowaid.metadata(buffer)
```
...
<td align="center">
    ...
    <td align="center">
      <a href="https://github.com/Renn-devBI">
